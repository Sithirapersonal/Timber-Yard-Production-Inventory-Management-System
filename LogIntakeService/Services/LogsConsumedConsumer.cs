...
/// <summary>
/// Background Kafka consumer for the 'logs-consumed' topic. Each event lists logs
/// that a freshly started saw job has allocated; this service flips those LogIds
/// (currently Status = 'InStock') to 'Consumed'. The flow is fire-and-forget:
/// SawmillService does not wait for us, and this consumer must never take the rest
/// of the service down — malformed messages are logged and skipped, and a broker
/// unreachable at startup is retried with exponential backoff instead of crashing
/// the host.
/// </summary>
public class LogsConsumedConsumer : BackgroundService
{
    private readonly string _bootstrapServers;
    private readonly string _topic;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<LogsConsumedConsumer> _logger;
    ...
    public LogsConsumedConsumer(
        IConfiguration configuration,
        IServiceScopeFactory scopeFactory,
        ILogger<LogsConsumedConsumer> logger)
    {
        _bootstrapServers = configuration["Kafka:BootstrapServers"] ?? "localhost:9092";
        _topic = configuration["Kafka:LogsConsumedTopic"] ?? "logs-consumed";
    ...
    private async Task RunConsumeLoopAsync(CancellationToken stoppingToken)
    {
        var config = new ConsumerConfig
        {
            ...
            GroupId = "logintake-logs-consumed",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Failed to process logs-consumed message {Key}", result.Message.Key);
            }
        }
    }
    ...
    public async Task ProcessPayloadAsync(string payload, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Skipping malformed logs-consumed payload: {Payload}", payload);
            return;
        }
        ...
        var rowsAffected = await repository.MarkLogsConsumedAsync(requestedIds);

        _logger.LogInformation(
            "Processed logs-consumed event for job {SawJobId}: consumed {Consumed}/{Requested} log(s)",
            evt.SawJobId, rowsAffected, requestedIds.Count);

        if (rowsAffected != requestedIds.Count)
        {
            // Useful signal, not an error: a log may already be Consumed (re-delivered
            // event) or be in some other state (removed). Log, don't throw.
            _logger.LogWarning(
                "Logs-consumed for job {SawJobId}: {RowsAffected}/{Requested} logs updated " +
                "(already Consumed, removed, or duplicate delivery)",
                evt.SawJobId, rowsAffected, requestedIds.Count);
        }
    }

    /// <summary>
    /// Deserializes a raw payload into a <see cref="LogsConsumedEvent"/>.
    /// Returns false (without throwing) for malformed JSON so the caller can log-and-skip.
    /// </summary>
    public static bool TryParsePayload(string payload, [NotNullWhen(true)] out LogsConsumedEvent? evt)
    {
        evt = null;
        if (string.IsNullOrWhiteSpace(payload)) return false;
        try
        {
            evt = JsonSerializer.Deserialize<LogsConsumedEvent>(payload, _jsonOpts);
